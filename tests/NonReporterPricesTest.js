...
describe("UniswapAnchoredView", () => {
  ...
  it("reverts fixed_eth prices if no ETH price", async () => {
    ...
    expect(call(oracle, "price", ["SAI"])).rejects.toRevert(
      "revert ETH price not set, cannot convert to dollars"
    );
  });
  ...
  it("reverts if ETH has no uniswap market", async () => {
    if (!coverage) {
      ...
      expect(
        deploy("UniswapAnchoredView", [0, 0, [ETH, SAI]])
  ...
  it("handles fixed_eth prices", async () => {
    if (!coverage) {
      ...
      const ETH = {
        ...
        priceSource: PriceSource.REPORTER,
        fixedPrice: 0,
        uniswapMarket: usdc_eth_pair._address,
        reporter: reporter.options.address,
        reporterMultiplier: uint(1e16),
        isUniswapReversed: true,
      };
      const SAI = {
      ...
      const oracle = await deploy("UniswapAnchoredView", [
        uint(20e16),
        60,
        [ETH, SAI],
      ]);
