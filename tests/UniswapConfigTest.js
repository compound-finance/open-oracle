...
describe('UniswapConfig', () => {
  it('basically works', async () => {
    ...
    const contract = await deploy('UniswapConfig', [[
      {cToken: address(1), underlying: address(0), symbolHash: keccak256('ETH'), baseUnit: uint(1e18), priceSource: 0, fixedPrice: 0, uniswapMarket: address(6), reporter: address(8), reporterMultiplier: uint(1e16), isUniswapReversed: false},
      {cToken: address(2), underlying: address(3), symbolHash: keccak256('BTC'), baseUnit: uint(1e18), priceSource: 1, fixedPrice: 1, uniswapMarket: address(7), reporter: address(9), reporterMultiplier: uint(1e16), isUniswapReversed: true},
      {cToken: unlistedButUnderlying._address, underlying: address(4), symbolHash: keccak256('REP'), baseUnit: uint(1e18), priceSource: 1, fixedPrice: 1, uniswapMarket: address(7), reporter: address(10), reporterMultiplier: uint(1e16), isUniswapReversed: true}
    ]]);
  ...
  it('checks gas', async () => {
    ...
    expect(tx9.gasUsed).toEqual(22913);
    ...
    expect(tx25.gasUsed).toEqual(23303);

    const cfgY = await call(contract, 'getTokenConfigBySymbol', ['y']);
    const txY = await send(contract, 'getTokenConfigBySymbol', ['y']);
    expect(cfgY.cToken).addrEquals(address(24));
    expect(cfgY.underlying).addrEquals(address(25));
    expect(txY.gasUsed).toEqual(25600);
    ...
    expect(txCT26.gasUsed).toEqual(25384);
    ...
    expect(txR26.gasUsed).toEqual(25385);
    ...
    expect(txU26.gasUsed).toEqual(25407);
  });
});